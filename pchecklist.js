...
var passwordCheckOld = function () 
{
   ...
   // Flag is my attempt to work around the problem: the code was not checking if the stringPassword matched all 497 values in the array.
   var flag = null;
  /* insert code
   * here
   */
   //alert(arrayBadPasswords.length);
   for (var a = 0; a < arrayBadPasswords.length; a++) {
         console.log(a);
        if (stringPassword != arrayBadPasswords[a]) {
         flag = true;
        } else {
          // $("output").value = "This password is not good.";
           alert("This password is not good.");
           flag = false;
           break;
        }
     };
  
// The flag must stay true throughout, hence the else if. Flag being true means stringPassword is not equal to any array values.
if (stringPassword.length <= 7 && flag == true) {
   alert("You must enter a password at least 8 characters long.");
} else if (flag == true) {
   alert("This password is good!");
}
